...
def psp_kernel(tau_syn: float, tau_mem: float, dt: float = 1.0, size: Optional[int] = None):
    if size is None:
    ...
    psp = conv1d(kernel_syn.reshape(1,1,-1), kernel_mem.flip(0).reshape(1,1,-1), padding=padding)
    return psp.squeeze()[:size]


def psp_kernels(tau_syn: List, tau_mem: float, dt: float = 1.0, size: Optional[int] = None):
    size = int(FACTOR*max(*tau_syn, tau_mem)/dt)
    kernel_syn = []
