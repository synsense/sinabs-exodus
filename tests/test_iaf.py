...
def test_sinabs_model():
    ...
    model = SinabsIAFModel(n_input_channels=n_input_channels, 
...
def test_slayer_model():
    batch_size, time_steps = 10, 100
    n_input_channels, n_output_classes = 16, 10
    model = SlayerIAFModel(n_input_channels=n_input_channels, 
                            n_output_classes=n_output_classes).cuda()
    input_data = torch.rand((batch_size, time_steps, n_input_channels)).cuda() * 1e5
    spike_output = model(input_data)
...
def test_slayer_sinabs_model_equal_output():
    ...
    for (sinabs_layer, slayer_layer) in zip(sinabs_model.linear_layers, slayer_model.linear_layers):
...
def test_slayer_vs_sinabs_compare_grads():
    batch_size, time_steps = 10, 100
    n_input_channels, n_output_classes = 16, 10
    sinabs_model = SinabsIAFModel(n_input_channels=n_input_channels, 
                            n_output_classes=n_output_classes).cuda()
    slayer_model = SlayerIAFModel(n_input_channels=n_input_channels, 
                            n_output_classes=n_output_classes).cuda()
    ...
    grads_sinabs = [p.grad.data.clone() for p in sinabs_model.parameters() if p.grad is not None]
    ...
    for (l_sin, l_slyr) in zip(slayer_model.spiking_layers, sinabs_model.spiking_layers):
